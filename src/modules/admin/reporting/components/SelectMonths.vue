<template>
    <Listbox v-model="selectedMonth">
        <div class="relative mt-1">
            <ListboxButton
                class="relative w-80 h-10 border-gray-500 cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left shadow-md focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
            ...
            <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100"
                ...
                <ListboxOptions
                    ...
                    <ListboxOption v-slot="{ active, selected }" v-for="month in months" :key="month" :value="month"
                        as="template">
                        <li :class="[
...
import { getAllMonths } from '../../composables/admin-report-composables';
...
watch(selectedMonth, () => {
    props.setMonthToNumber(months.value.findIndex(month => month == selectedMonth.value) + 1)
})
