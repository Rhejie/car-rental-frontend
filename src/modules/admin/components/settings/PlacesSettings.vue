...
import CreatePlaceModal from '../modals/CreatePlaceModal.vue'
import GNotification from '@/components/GNotification.vue';
import { loadPlaces } from '../composables/place-composables';
const places = ref([])
...
const fetchPlaces = async () => {
    loading.value = true
    const { load, data, total } = loadPlaces(params.value)
    await load();
...
const handleCloseAddPlaceModal = (status) => {
...
<template>
    <CreatePlaceModal 
        ...
        @closeModal="handleCloseAddPlaceModal" 
        @updatePlace="handleUpdatePlace"
        @savePlace="handleNewPlace"/>
    <GNotification :show-notif="showNotif"/>
    <div class="mx-auto max-w-4xl py-10">
        <div class="bg-white p-2 shadow-md rounded-lg">
            <h1 class="text-3xl font-bold tracking-tight text-gray-700">Places</h1>
                                    ...
                                    <tbody class="divide-y divide-gray-200 bg-white" v-loading="loading">
                                        <tr v-for="place in places" :key="place.email">
                                            ...
                                            <td
                                                ...
                                                <button class="text-indigo-600 hover:text-indigo-900 mr-2" @click="handleClickEdit(place)">Edit</button>
