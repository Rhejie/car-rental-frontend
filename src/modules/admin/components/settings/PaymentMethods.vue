...
import CreatePaymentMethodModal from '../modals/CreatePaymentMethodModal.vue'
import GNotification from '@/components/GNotification.vue';
import { loadPaymentMethods } from '../composables/payment-method-composables';
...
const fetchPaymentMethods = async () => {
    loading.value = true
    const { load, data, total } = loadPaymentMethods(params.value)
    await load();
...
const handleCloseAddPaymentMethodModal = (status) => {
    openModal.value = false
}

const handleNewPaymentMethod = (paymentMethod) => {
    paymentMethods.value.unshift(paymentMethod)
    openModal.value = false
...
const handleUpdatePaymentMethod = (paymentMethod) => {
    ...
    paymentMethods.value.map(id => {
        if(id.id == paymentMethod.id){
            for(let key in paymentMethod) {
                id[key] = paymentMethod[key]
            }
        }
...
const handleClickEdit = (paymentMethod) => {
    openModal.value = true
    selectedItem.value = paymentMethod
}
...
<template>
    <CreatePaymentMethodModal 
        ...
        @closeModal="handleCloseAddPaymentMethodModal" 
        @updatePaymentMethod="handleUpdatePaymentMethod"
        @savePaymentMethod="handleNewPaymentMethod"/>
    <GNotification :show-notif="showNotif"/>
    <div class="mx-auto max-w-4xl py-10">
        <div class="bg-white p-2 shadow-md rounded-lg">
            <h1 class="text-3xl font-bold tracking-tight text-blue-gray-900">Payment Methods</h1>
            <div class="px-4 sm:px-6 lg:px-8">
                <div class="sm:flex sm:items-center">
                    ...
                    <div class=" sm:flex-none">
                        <button type="button"
                            @click="handleClickAddPaymentMethod"
                            ...
                            <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                                <table class="min-w-full divide-y divide-gray-300">
                                    ...
                                    <tbody class="divide-y divide-gray-200 bg-white" v-loading="loading">
                                        <tr v-for="paymentMethod in paymentMethods" :key="paymentMethod.email">
                                            <td
                                            ...
                                            <td
                                                ...
                                                <button class="text-indigo-600 hover:text-indigo-900 mr-2" @click="handleClickEdit(paymentMethod)">Edit</button>
