...
import { storeDriver, updateDriver } from '../composables/driver-composables'
...
const emit = defineEmits(['closeModal', 'saveDriver', 'updateDriver'])
...
const handleUpdateDriver = async () => {
    loading.value = true
    const {data, update, errorData} = updateDriver(driver.value);
    await update();
    errorValue.value = errorData.value
    loading.value = false
    if(!loading.value && !errorValue.value, !errorData.value) {
        emit('updateDriver', data.value)
    }
}

const handleStoreRole = async () => {

    if(driver.value && driver.value.id) {
        handleUpdateDriver()
        return;
    }

    loading.value = true
    const {data, post, errorData} = storeDriver(driver.value);
    await post();
    errorValue.value = errorData.value
    loading.value = false
    if(!loading.value && !errorValue.value, !errorData.value) {
        emit('saveDriver', data.value)
    }
                ...
                <div class="flex min-h-full items-end justify-center sm:items-center p-4 sm:p-0">
                    <TransitionChild as="template" enter="ease-out duration-300"
                        ...
                        <DialogPanel
                            ...
                            <DialogTitle as="h3" class="text-lg font-medium leading-6 text-gray-700 p-2">{{ driver && driver.id ? 'Update' : 'Create'}}  Driver</DialogTitle>
                                        ...
                                        <div class="bg-white px-4 py-5 sm:p-6">
                                            <div class="grid grid-cols-6 gap-6">
                                                <div class="col-span-12">
                                                    ...
                                                    <input type="text" name="first-name" v-model="driver.name"
                                                        class="mt-1 block w-full rounded-md border h-8 border-gray-100 focus:border-indigo-100 focus:ring-indigo-100 pl-2 sm:text-sm" placeholder="Full Name" />
                                                    <span 
