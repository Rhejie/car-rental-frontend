...
import { storeColor, updateColor } from '../composables/color-composables'
...
const emit = defineEmits(['closeModal', 'saveColor', 'updateColor'])
...
const handleUpdateColor = async () => {
    loading.value = true
    const {data, update, errorData} = updateColor(color.value);
    await update();
    errorValue.value = errorData.value
    loading.value = false
    if(!loading.value && !errorValue.value, !errorData.value) {
        emit('updateColor', data.value)
    }
}

const handleStoreRole = async () => {

    if(color.value && color.value.id) {
        handleUpdateColor()
        return;
    }

    loading.value = true
    const {data, post, errorData} = storeColor(color.value);
    await post();
    errorValue.value = errorData.value
    loading.value = false
    if(!loading.value && !errorValue.value, !errorData.value) {
                ...
                <div class="flex min-h-full items-end justify-center sm:items-center p-4 sm:p-0">
                    <TransitionChild as="template" enter="ease-out duration-300"
                        ...
                        <DialogPanel
                            ...
                            <DialogTitle as="h3" class="text-lg font-medium leading-6 text-gray-700 p-2">{{ color && color.id ? 'Update' : 'Create'}}  Color</DialogTitle>
                                        ...
                                        <div class="bg-white px-4 py-5 sm:p-6">
                                            <div class="grid grid-cols-6 gap-6">
                                                <div class="col-span-12">
                                                    ...
                                                    <input type="text" name="first-name" v-model="color.name"
                                                        class="mt-1 block w-full rounded-md border h-8 border-gray-100 focus:border-indigo-100 focus:ring-indigo-100 pl-2 sm:text-sm" placeholder="color Name" />
                                                    <span 
